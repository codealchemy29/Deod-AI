...
import { Search, Brain, Palette, Video, Music, Code, Bot, ExternalLink, CheckCircle2, Filter, Grid3X3, Star, } from "lucide-react";
/* =======================
    DATA
======================= */
var categories = [
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input type="search" placeholder="Search AI tools..." className="pl-10 bg-muted/50 focus-visible:ring-[#1e3a8a]" value={searchQuery} onChange={function (e) { return setSearchQuery(e.target.value); }}/>
            </div>
          ...
          <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-hide">
            {categories.map(function (cat) { return (<Button key={cat.id} size="sm" variant={selectedCategory === cat.id ? "default" : "outline"} className={selectedCategory === cat.id
                ? "bg-[#1e3a8a] hover:bg-[#1e3a8a]/90 text-white"
                : "border-border hover:bg-[#1e3a8a]/5 hover:text-[#1e3a8a]"} onClick={function () { return setSelectedCategory(cat.id); }}>
      ...
      <section className="py-10">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-muted-foreground mb-6">
            {isLoading ? "Loading..." : "Showing ".concat(filteredTools.length, " tools")}
          </p>

          {error && (<div className="text-center py-12 text-destructive">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            : filteredTools.map(function (tool) {
                ...
                return (<Card key={tool.id} className="bg-card border-border hover:shadow-md dark:hover:shadow-[#1e3a8a]/10 transition group">
                      <CardContent className="p-6">
                        <div className="flex gap-4 mb-4">
                          <div className={"w-12 h-12 rounded-lg bg-gradient-to-r ".concat(getCategoryColor(tool.category), " flex items-center justify-center shadow-sm")}>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              {tool.verified && (<CheckCircle2 className="h-4 w-4 text-teal-600 dark:text-teal-400"/>)}
                            </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-border/50">
                          ...
                          <Badge className={tool.pricing.toLowerCase().includes("free")
                        ...
                        <Button variant="outline" className="w-full mt-4 border-[#1e3a8a] text-[#1e3a8a] hover:bg-[#1e3a8a] hover:text-white transition-all active:scale-[0.98]">
                          View Tool
          ...
          {!isLoading && filteredTools.length === 0 && (<div className="text-center py-20 bg-card rounded-2xl border border-dashed border-border mt-10">
              ...
              <Button variant="outline" className="mt-4 text-[#1e3a8a]" onClick={function () {
                setSearchQuery("");
